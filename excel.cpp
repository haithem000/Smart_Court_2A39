...
int Excel::export2Excel()
{
    if(fieldList.size() <= 0)
    {
        qDebug() << "Excel::export2Excel failed: No fields defined.";
        return -1;
    }
    ...
    if(!db.isValid())
    {
        qDebug() << "Excel::export2Excel failed: QODBC not supported.";
        return -2;
    }
    // set the dsn string
    QString dsn = QString("DRIVER={Microsoft Excel Driver (*.xls)};DSN='';FIRSTROWHASNAMES=1;READONLY=FALSE;CREATE_DB=\"%1\";DBQ=%2").
    ...
    if(!db.open())
    {
        qDebug() << "Excel::export2Excel failed: Create Excel file failed by DRIVER={Microsoft Excel Driver (*.xls)}.";
        //QSqlDatabase::removeDatabase("excelexport");
        return -3;
    }
    ...
    if(!query.exec())
    {
        qDebug() << "Excel::export2Excel failed: Create Excel sheet failed.";
        //db.close();
        //QSqlDatabase::removeDatabase("excelexport");
        return -4;
    }
